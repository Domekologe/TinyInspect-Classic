...
local function AppendToGameTooltip(guid, ilevel, spec, weaponLevel, isArtifact)
    ...
    if (weaponLevel and weaponLevel > 0 and TinyInspectClassicDB.EnableMouseWeaponLevel) then
...
if (GameTooltip.ProcessInfo) then
    hooksecurefunc(GameTooltip, "ProcessInfo", function(self, info)
        ...
        if (TinyInspectClassicDB and (TinyInspectClassicDB.EnableMouseItemLevel or TinyInspectClassicDB.EnableMouseSpecialization)) then
...
LibEvent:attachTrigger("UNIT_INSPECT_READY", function(self, data)
    if (TinyInspectClassicDB and not TinyInspectClassicDB.EnableMouseItemLevel) then return end
